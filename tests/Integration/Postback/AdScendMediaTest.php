...
class AdScendMediaTest extends TestCase
{
    ...
    protected function setUp(): void
    {
        parent::setUp();
        $this->addWhitelistIp();
        $this->createTestUser('adscendtest');
    }

    protected function tearDown(): void
    {
        $this->removeTestUser('adscendtest');
    ...
    {
        $handler = new \FlyCash\Postback\Handlers\AdScendMediaHandler($this->getPDO());
        $ref = new \ReflectionClass($handler);
        $parseRequest = $ref->getMethod('parseRequest');

        $_GET['sub1'] = 'adscendtest';
        $_GET['rate'] = '250';
        $_GET['offerid'] = 'offer_abc_123';

        $parseRequest->invoke($handler);

        $this->assertSame('adscendtest', $ref->getProperty('userId')->getValue($handler));
        $this->assertSame(250, $ref->getProperty('amount')->getValue($handler));
        $this->assertSame('offer_abc_123', $ref->getProperty('transactionId')->getValue($handler));
    }
    ...
    public function test_should_process_returns_true_for_valid_amount(): void
    {
        ...
        $ref->getProperty('amount')->setValue($handler, 50);
        $ref->getProperty('userId')->setValue($handler, 'adscendtest');
