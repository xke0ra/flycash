...
class WannadsTest extends TestCase
{
    ...
    {
        $handler = new \FlyCash\Postback\Handlers\WannadsHandler($this->getPDO());
    ...
    public function test_parse_request_extracts_subid_and_reward(): void
    {
        $handler = new \FlyCash\Postback\Handlers\WannadsHandler($this->getPDO());
        $ref = new \ReflectionClass($handler);
        ...
        $_GET['subId'] = 'wannadstest';
        $_GET['reward'] = '250';
        $_GET['status'] = '1';

        $parseRequest->invoke($handler);

        $this->assertSame('wannadstest', $userIdProp->getValue($handler));
        $this->assertSame(250, $amountProp->getValue($handler));
    }
}
