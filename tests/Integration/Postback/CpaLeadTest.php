...
class CpaLeadTest extends TestCase
{
    ...
    protected function setUp(): void
    {
        parent::setUp();
        $this->addWhitelistIp();
        $this->createTestUser('cpaleadtest');
    }

    protected function tearDown(): void
    {
        $this->removeTestUser('cpaleadtest');
    ...
    {
        $handler = new \FlyCash\Postback\Handlers\CpaLeadHandler($this->getPDO());
        $ref = new \ReflectionClass($handler);
        $parseRequest = $ref->getMethod('parseRequest');

        $_GET['subid2'] = 'cpaleadtest';
        $_GET['virtual_currency'] = '300';

        $parseRequest->invoke($handler);

        $this->assertSame('cpaleadtest', $ref->getProperty('userId')->getValue($handler));
        $this->assertSame(300, $ref->getProperty('amount')->getValue($handler));
    }

    public function test_validate_user_rejects_empty_user_id(): void
    {
        $handler = new \FlyCash\Postback\Handlers\CpaLeadHandler($this->getPDO());
        $ref = new \ReflectionClass($handler);
    ...
    public function test_validate_user_accepts_valid_user(): void
    {
        ...
        $ref->getProperty('userId')->setValue($handler, 'cpaleadtest');
        $ref->getProperty('amount')->setValue($handler, 100);
