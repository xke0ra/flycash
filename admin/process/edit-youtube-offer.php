    ...
    if (!admin::isSession()) {
		...
		if($image["offer_image"]){
			...
			if($upload){
    			...
    			if($stmt->execute(array(':name' => $name, ':subtitle' => $subtitle, ':offerwall_url' => $offerwall_url, ':duration' => $duration, ':points' => $points, ':urlfinal' => $urlfinal, ':ID' => $ID))){ $result = true; }
    			...
    			if($stmt->execute(array(':name' => $name, ':subtitle' => $subtitle, ':offerwall_url' => $offerwall_url, ':duration' => $duration, ':points' => $points, ':ID' => $ID))){ $result = true; }
